...
export const MODEL_FILES = [
  ...
  { id: "bayonetLong" as const, url: "/models/bayonet-long.glb", bytes: 158_192, path: "/models/", hint: "长刺刀" },
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (cancelled) return;
        useStudio.setState({ loadProgress: 98, loadHint: "解析长刺刀" });
        ...
        setScenes({ character, intestines, pelvis, arm, bayonet, bayonetLong, room });
      } catch (err) {
        if (cancelled) return;
